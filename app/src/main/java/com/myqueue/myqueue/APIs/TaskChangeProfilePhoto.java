 ...
 */
public abstract class TaskChangeProfilePhoto extends AsyncTask<APIUpdatePhotoProfile, Void, Boolean>{
    ...
    public TaskChangeProfilePhoto(Context context) {
    ...
    @Override
    protected Boolean doInBackground(APIUpdatePhotoProfile... APIUpdatePhotoProfile) {
        API api = restAdapter.create(API.class);

        com.myqueue.myqueue.Models.APIUpdatePhotoProfile param = APIUpdatePhotoProfile[0];
        try {
            response = api.ChangeProfilePicture(param.getUserid(),param.getPhotoProfile(),param.getUrlPhotoProfile());
            return true;
        }
